...
Bug reports are sent by `api/bugreport.js` to the shared Politikum bugs channel (`-5260075189`). It uses `TELEGRAM_BOT_TOKEN` directly when configured; otherwise it relays through the already-configured Politikum bug endpoint, so a separate Telegram secret is not required for this project.
